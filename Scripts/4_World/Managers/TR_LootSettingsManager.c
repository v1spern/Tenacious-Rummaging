...
class TR_LootSettingsManager
{
        ...
        if (FileExist(path))
        {
            ...
            if (s_Data && s_Data.Categories)
            {
                foreach (string k, TR_CategorySettings cs : s_Data.Categories)
                {
                    ...
                    // Backfill containers/fields (mark mutated to persist)
            ...
            if (mutated)
            {
                ...
                TR_Debug.Log("[LootSettings] Backfill detected; wrote updated LootSettings.json");
            }
        }
        else
        {
    ...
    static void SaveManual(TR_LootSettingsData data)
    {
        ...
        // Ensure categories have events/fields before saving
    ...
    // Ensure category has Events and backfill
    ...
    // Backfill missing sub-objects and missing FlavorText fields.
    ...
    static void ValidateGroups()
    {
        ...
        foreach (string g : groups)
        {
            ...
            {
                TR_Debug.Warn("LootSettings.json missing settings for group '" + g + "'. Using defaults.");
            }
        }
        ...
        if (s_Data.Categories)
        {
            foreach (string key, TR_CategorySettings settings : s_Data.Categories)
            {
                if (!groups.Find(key))
                {
                    TR_Debug.Warn("LootSettings.json defines category '" + key + "' which has no matching loot group in LootGroups.json.");
                }
            }
        }
    }
